...
- `PlayerRoot`：需要接入战斗表现时，在 `PlayerModuleController` 的 `战斗表现 / Combat Reaction Profile` 中拖 `TPCCombatReactionProfile`。
...
| `战斗表现配置` | 拖 `TPCCombatReactionProfile`，配置轻受击、重受击、格挡、击倒、死亡等动画 | 接 Combat 时不建议 | `TryPlayCombatReaction` 返回 false，不播放 Combat 受击/死亡表现 |
| `Use External Sfx Bridge` | 接入 NiumaAudio 时开启 | 可以 | 关闭时仍使用 TPC 原本本地音效 |

常用公开入口：

- `DisableControl(reason)` / `EnableControl(reason)`：按原因冻结或恢复玩家输入。
- `Teleport(position, rotation)`：场景切换、读档、复活时移动玩家。
- `TryPlayCombatReaction(request)`：Combat / Bridge 请求播放受击、格挡、击倒、死亡表现。
- `ReleaseCombatReactionControl()`：如果某个战斗表现请求冻结了输入，可由外部在表现结束后释放 `CombatReaction` 输入锁。死亡表现通常不释放。

### TPCCombatReactionProfile
建议资产位置：`Assets/Game/Config/TPC/CombatReaction` 或角色配置目录。

创建方式：Project 面板右键 `Create/NiumaTPC/战斗表现配置`。

| 字段 | 怎么填 | 可否留空 | 不填会怎样 |
| --- | --- | --- | --- |
| `默认受击表现` | 展开后填写 `动画数据`，在 MotionClipData 的 `Clip` 中拖动画片段 | 可以 | 找不到指定反应时没有兜底动作 |
| `死亡表现` | 展开后填写 `动画数据`，在 MotionClipData 的 `Clip` 中拖死亡动画片段 | Combat 死亡表现不建议 | `死亡` 请求无法播放死亡动作 |
| `按类型覆盖列表` | 配置轻受击、重受击、格挡、击倒等类型到动画的映射 | 可以 | 对应类型会回退默认受击表现或播放失败 |

`动作优先级` 越高越容易打断当前 Override 动作。`冻结输入` 轻受击建议关闭，击倒和死亡可以开启。`应用重力` 通常保持开启，避免 Override 动作期间悬空。

`表现类型` 下拉项已汉化为 `无 / 轻受击 / 重受击 / 格挡 / 击倒 / 死亡`；代码里仍保留英文枚举名，方便桥接层稳定调用。

动画烘焙说明：
- `动画数据` 使用 TPC 原有 `MotionClipData`，会被 `Tools/NiumaTPC/RootMotionBaker v2.3.5` 扫描并写入速度、旋转等烘焙曲线。
- 打开 `Tools/NiumaTPC/RootMotionBaker v2.3.5` 后，目标资产可以直接拖 `TPCCombatReactionProfile`，也可以拖包含它的角色配置资产。
- 战斗表现不提供单独 `AnimationClip` 兜底入口，统一按 TPC 其他可烘焙动画配置方式填写 `MotionClipData`。
- 播放战斗表现时，`OverrideState` 会关闭 Animator RootMotion，改用 TPC `MotionDriver` 的烘焙曲线驱动位移，避免根运动造成不可控位移。

旧版死亡链路：
- 旧 TPC 内部死亡由 `HealthArbiter` 判定血量后切换到 `PlayerDeathState`。
- `PlayerDeathState` 播放的是角色 `CoreSO.DeathAnim`，并在旧逻辑里硬编码处理死亡后的回收/流程。
- 新增的 `TPCCombatReactionProfile.死亡表现` 是给外部 `NiumaCombat` / Bridge 使用的表现入口，不会自动删除旧 `PlayerDeathState`；后续 Combat 桥接稳定后，再决定旧链路是否退场。

### Combat 表现接入边界

`TPCCombatBridge` 已在 `NiumaCombat.Runtime/TPCBridge` 中落地。它应挂在 `PlayerRoot/CombatBridge` 或统一桥接物体上，并拖 `PlayerModuleController` 与当前角色的 `CombatActorIdentity`。
...
CombatResult
-> TPCCombatBridge
-> TPCCombatReactionRequest
-> PlayerModuleController.TryPlayCombatReaction
-> NiumaCharacterController.RequestOverride
-> ActionArbiter / OverrideState
```

`TPCCombatReactionRequest` 当前会接收 `HitPoint`、`HitDirection`、`Force`、`StaggerSeconds`、`KnockbackDistance`、`HitVfxCueId`、`HitAudioCueId`。第一版 TPC 只直接使用动作类型、优先级、输入冻结和烘焙动作数据，其余字段先作为后续受击状态、特效、音频桥接的表现上下文保留。

桥接层不要直接调用 `NiumaCharacterController.RequestOverride`，也不要直接切换 TPC 内部 HFSM 状态。

旧 `HealthArbiter / DamageRequest` 只作为 TPC 内部旧测试路径保留，不作为正式 Combat 扣血入口。正式伤害、治疗、死亡事实由 `NiumaCombat + NiumaAttribute` 负责，TPC 只播放表现。

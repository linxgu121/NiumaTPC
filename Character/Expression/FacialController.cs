...
namespace NiumaTPC.Character.Expression
{
    ...
    public sealed class FacialController
    {
        ...
        public FacialController(NiumaCharacterController player)
        {
            _player = player;
        ...
        public void Update()
        {
            if (_player == null || _player.AnimationFacade == null) return;
            ...
            if (!_initialized)
            {
                ...
                if (_config != null && _config.Core != null)
                    _player.AnimationFacade.SetLayerMask(FacialLayer, _config.Core.FacialMask);
            ...
            {
                _player.AnimationFacade.SetLayerWeight(FacialLayer, 0f);
                return;
            }

            _player.AnimationFacade.SetLayerWeight(FacialLayer, 1f);
        ...
        private void PlayTransientExpression(PlayerFacialEvent evt, ClipTransition transition, float fade)
        {
            ...
            // 播放表情动画
            _player.AnimationFacade.PlayTransition(transition, options);

            // 注册动画结束回调 用于过期时自动回归
            _player.AnimationFacade.SetOnEndCallback(() =>
            {
        ...
        private void ClearLock(bool clearCallback)
        {
            ...
            if (clearCallback)
            {
                _player.AnimationFacade.ClearOnEndCallback(FacialLayer);
            }
        }
        ...
        private void PlayBaseExpression(float fade = 0.25f)
        {
            ...
            _player.AnimationFacade.PlayTransition(_baseExpression, options);
            _player.AnimationFacade.ClearOnEndCallback(FacialLayer);
        }
    }
}
